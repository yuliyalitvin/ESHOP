...
/**
 * 
 * Feld fuer Mitarbeiter um neue Artikel einzufuegen oder bestand zu erhoehen
 * 
...
public class FuegeArtikelHinzuPanel extends JPanel {
	...
	public FuegeArtikelHinzuPanel(EShopInterface verwaltung, Benutzer benutzer, ArtikelHinzufuegenListener listener) {
	...
	private void erstelleUI() {
		...
		this.add(new JLabel("Anzahl hinzufuegen: "));
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				if (!nummer.isEmpty() && !name.isEmpty() && !preis.isEmpty() && !anzahl.isEmpty()) {
					try {
						...
						artikel = eShopInterface.fuegeArtikelEin(name, nr, pr, anz, massengutartikel, pack);
						eShopInterface.addHistorieEreignis(nr, anz, 1, aktuellerBenutzer.getBenutzerIndex());
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				if (!nummerBestand.isEmpty() && !anzahlBestand.isEmpty()) {
					try {
						...
						eShopInterface.bestandErhoehen(plus, artikel);
						eShopInterface.addHistorieEreignis(nr, plus, 2, aktuellerBenutzer.getBenutzerIndex());
