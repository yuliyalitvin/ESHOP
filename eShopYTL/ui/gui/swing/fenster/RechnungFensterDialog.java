	...
	class GraphOberflaeche extends Canvas {
		...
		private void plot(Graphics graph) {
			...
			for (Artikel artikel : gekaufteArtikel) {
				...
				graph.drawString("" + artikel.getArtikelNummer(), xNummer + 3, y);
				graph.drawString(artikel.getArtikelName(), xName + 3, y);
				String stueck = "" + artikel.getStueck();
				stringWidth = fm.stringWidth(stueck);
				graph.drawString(stueck, xPreis - 3 - stringWidth, y);

				String preis = "" + artikel.getPreis();
